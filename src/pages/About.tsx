import { memo, useState, useMemo, useRef, useEffect, useCallback, type CSSProperties } from 'react';
...
import { PageLayout, LoadingState, ErrorState, EmptyState, Tag, KawaiiStar, KawaiiBow, KawaiiSparkle, KawaiiHeart } from '../components';
...
function StickyBioNote({ bio, rankHex, editable, tilt }: { bio?: string; rankHex: string; editable: boolean; tilt: number }) {
  ...
  const updateScroll = useCallback(() => {
    ...
    setScrollState((prev) => (prev.canScroll === canScroll && prev.atBottom === atBottom ? prev : { canScroll, atBottom }));
  }, []);
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;
    ...
    ro.observe(el);
    let cancelled = false;
    void document.fonts?.ready?.then(() => { if (!cancelled) updateScroll(); });
  ...
  const stickyStyle: CSSProperties = {
    background: stickyBg,
    boxShadow: '2px 5px 13px -3px rgba(0, 0, 0, 0.30), inset 0 -14px 18px -14px rgba(90, 70, 25, 0.18)',
  };
  ...
  const shellCls = 'relative z-20 shrink-0 -ml-4 sm:-ml-5 mt-4 sm:mt-6 w-44 sm:w-56';
  const shellStyle: CSSProperties = { transform: `rotate(${tilt}deg)` };
  const pin = (
    <span className="pushpin absolute -top-2 left-1/2 -translate-x-1/2 z-10" style={{ '--pin': rankHex } as CSSProperties} aria-hidden="true" />
  );
    ...
    return (
      <div className={shellCls} style={shellStyle}>
        ...
        <div className="rounded-[3px] p-3.5" style={stickyStyle}>
          ...
          {mutation.isError && <p className="mt-1 font-soft text-[11px] text-[#a23a2a]">couldn't save, kupo</p>}
        </div>
      </div>
    );
  }
    ...
    <div className={shellCls} style={shellStyle}>
      ...
      <div className="relative">
        <div
          ...
          style={stickyStyle}
        >
          <p className={`font-accent text-[17px] sm:text-lg leading-snug whitespace-pre-line ${bio ? 'text-[#463c2e]' : 'text-[#463c2e]/55'}`}>
        ...
        {scrollState.canScroll && !scrollState.atBottom && (
          <div
            ...
            style={{ background: `linear-gradient(to bottom, transparent, ${stickyBg} 78%)` }}
            aria-hidden="true"
          >
    ...
    <article className="relative flex items-start">
      ...
      >
        <span
          ...
          style={{ background: `repeating-linear-gradient(45deg, color-mix(in srgb, ${rankColor.hex} 45%, transparent) 0 5px, color-mix(in srgb, ${rankColor.hex} 24%, transparent) 5px 10px)` }}
          aria-hidden="true"
        />
        <div className="surface p-2 pb-2.5 w-28 sm:w-32">
          <div className="relative aspect-square rounded-lg overflow-hidden bg-[var(--bg)]">
            ...
            >
              <RankIcon className="w-3 h-3" style={{ color: rankColor.hex }} aria-hidden="true" />
            </div>
            <span className="absolute inset-0 flex items-center justify-center bg-black/25 opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden="true">
          ...
          <div className="flex justify-center mt-1.5">
            <Tag color={rankColor.hex} icon={<RankIcon className="w-3 h-3" aria-hidden="true" />}>
              {shortRank}
            </Tag>
          </div>
          {(isLeader || member.recentlyPromoted || isCurrentUser) && (
            <div className="flex flex-wrap items-center justify-center gap-x-1.5 gap-y-0.5 mt-1.5 text-[10px] font-display font-bold leading-none">
              {isLeader && <span className="text-[var(--secondary)]">leads</span>}
              {member.recentlyPromoted && (
                <span className="inline-flex items-center gap-0.5 text-[color:color-mix(in_srgb,var(--accent)_70%,var(--text))]">
                  <KawaiiSparkle className="w-2.5 h-2.5" />promoted
                </span>
              )}
              {isCurrentUser && <span className="text-[var(--primary)]">that's you!</span>}
            </div>
          )}
        </div>
      </a>
      ...
      <StickyBioNote bio={member.biography} rankHex={rankColor.hex} editable={isOwnEditable} tilt={tilt.note} />
    </article>
  );
});
      ...
      <div className="corkboard relative px-3.5 py-7 sm:px-6 sm:py-9 md:px-9 md:py-11">
        ...
        <header className="relative w-fit mx-auto mb-7 sm:mb-9 text-center animate-[fadeSlideIn_0.4s_ease-out]">
          ...
          <div className="surface paper -rotate-1 px-7 sm:px-12 py-5 sm:py-6">
            ...
            <p className="eyebrow-script text-lg sm:text-2xl text-[var(--secondary)]/90 mb-1">~ pull up a chair, kupo ~</p>
        ...
        >
          <span className="pushpin absolute -top-2 left-10 z-10" style={{ '--pin': 'var(--primary)' } as CSSProperties} aria-hidden="true" />
          <span className="pushpin absolute -top-2 right-10 z-10" style={{ '--pin': 'var(--accent)' } as CSSProperties} aria-hidden="true" />
          ...
          >
            ...
            {staff.length > 0 && (
              <span
                ...
                style={{ backgroundColor: 'color-mix(in srgb, var(--primary) 20%, var(--card))', color: 'var(--primary)' }}
              >
                {staff.length}
              </span>
            )}
        ...
        {/* ── Staff scrapbook ───────────────────────────────────────────────── */}
        {isLoading ? (
          <div className="paper"><LoadingState message="Rounding everyone up, kupo..." /></div>
        ) : isError ? (
