...
const defaultPositions: SparklePosition[] = [
  ...
  { left: '92%', top: '18%', size: 'w-5 h-5', color: 'text-[var(--secondary)]' },
...
const minimalPositions: SparklePosition[] = [
  ...
  { left: '92%', top: '18%', size: 'w-4 h-4', color: 'text-[var(--secondary)]' },
  { left: '12%', top: '65%', size: 'w-4 h-4', color: 'text-[var(--secondary)]' },
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {sparkles.map((sparkle, i) => (
        ...
        >
          {i % 3 === 0 ? (
            <Star className={`${sparkle.size ?? 'w-4 h-4'} ${sparkle.color ?? 'text-[var(--primary)]'} fill-current`} />
          ) : (
            <Sparkles className={`${sparkle.size ?? 'w-4 h-4'} ${sparkle.color ?? 'text-[var(--primary)]'}`} />
          )}
        </div>
      ))}
    </div>
