...
// CSS animations for entry + hover tilt so it all stays on the compositor
export const MemberCard = memo(function MemberCard({
  ...
  // deterministic per-card jitter so each card sits at a slightly different angle
  const baseTilt = ((index * 7 + 3) % 5) - 2; // -2 to 2 degrees
  const hoverTilt = ((index * 13 + 7) % 7) - 3; // -3 to 3 degrees
  const hoverY = -(((index * 5 + 3) % 8) + 12); // -12 to -19px
  const hoverX = ((index * 11 + 2) % 7) - 3; // -3 to 3px
  ...
  return (
    ...
    >
      {/* hover transforms live on this inner wrapper so the parent's fadeSlideIn
          entrance transform doesn't stomp them */}
      <div
      ...
      >
        ...
        {/* z-10 keeps the tape above the card surface */}
        <svg
