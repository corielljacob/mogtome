...
import {
  ...
  getEventTypeConfig,
} from "@/features/chronicle/eventTypes";
...
describe("eventTypes", () => {
  ...
  describe("getEventTypeConfig", () => {
    it("returns correct config for known event types", () => {
      expect(getEventTypeConfig("MemberJoined")).toBe(
        EVENT_TYPE_CONFIG.MemberJoined,
      );
      expect(getEventTypeConfig("MemberRejoined")).toBe(
        EVENT_TYPE_CONFIG.MemberRejoined,
      );
      expect(getEventTypeConfig("NameChanged")).toBe(
        EVENT_TYPE_CONFIG.NameChanged,
      );
      expect(getEventTypeConfig("RankPromoted")).toBe(
        EVENT_TYPE_CONFIG.RankPromoted,
      );
      expect(getEventTypeConfig("Announcement")).toBe(
        EVENT_TYPE_CONFIG.Announcement,
      );
    });
    ...
    it("returns default config for unknown event types", () => {
      ...
      expect(getEventTypeConfig("SomeRandomEvent")).toBe(
        DEFAULT_EVENT_TYPE_CONFIG,
      );
    });
  });
